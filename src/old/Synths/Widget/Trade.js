...
export const Trade = ({ toggle, ticker }) => {
  ...
  const daiBalance = '' // useTokenBalance(ContractMapping["DAI"])
  const longBalance = ''
  const shortBalance = ''
  const [ estimatedPricePerDai, setEstimatedPricePerDai ] = useState(209)
  const [ availableFromBalance, setAvailableFromBalance ] = useState('0')
  ...
  useEffect(() => {
    if (currentDirection === 'open') {
      return setAvailableFromBalance(daiBalance)
    } else {
      return setAvailableFromBalance((currentType === 'long') ? longBalance : shortBalance)
    }
  }, [daiBalance, longBalance, shortBalance])
